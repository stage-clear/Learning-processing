...
`attract()` 関数を `Mover` クラスにコピーします.

```processing
class Mover {
  // 既存のコードを追加
 
  // どのようにして別の Mover を引き付けるかを指示
  PVector attract(Mover m) {
    PVector force = PVector.sub(location, m.location);
    float distance = force.mag();
...
void draw() {
  ...
  for (int i = 0; i < movers.length; i++) {
    for (int j = 0; j < movers.length; j++) {
      if (i != j) {
        PVector force = movers[ij.attract(movers[i]);
        movers[i].applyForce(force);
      }
    }
    movers[i].update();
...
- [ ] Exercise 2.10
  - 例2.8の引力を反発力に変えてください。すべての Mover オブジェクトがマウスに引き付けられ、オブジェクト同士は反発し合う例を作成してみましょう。力の相対的な強さのバランスの取り方、そして力の計算に距離を最も効果的に使用する方法を考えてください。
